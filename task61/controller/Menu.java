...
//        library = LibraryCreator.createLibrary();
    ...
    private void processFileOption(String statement) {
        FileOption fileOption = FileOption.values()[(Integer.parseInt(statement)) - 1];
        switch (fileOption) {
            case CREATE:
                break;
            case LOAD:
    ...
    private void saveLibrary(){
        address = firstSave?inputValue(menuView.REQUEST_SAVE_FILE, PATTERN):address;
        firstSave=!fileController.saveToFile(address,library);
    }
